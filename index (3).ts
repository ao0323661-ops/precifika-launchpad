...
import { Plus, Search, CreditCard, Calendar, AlertCircle } from "lucide-react";
...
function SubscriptionsPage() {
  ...
  const summary = useMemo(() => {
    ...
    const renewable = active.filter((sub) => !sub.cancel_at_period_end);
    const resolved = active.length + canceled.length;

    return {
      renewalRate: resolved > 0 ? (renewable.length / resolved) * 100 : 0,
      churnRate: resolved > 0 ? (canceled.length / resolved) * 100 : 0,
      monthlyRevenue: active.reduce((sum, sub) => sum + Number(sub.amount), 0),
    };
  }, [subscriptions]);
      ...
      <div className="grid gap-6 md:grid-cols-3">
        <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <p className="text-2xl font-bold text-slate-900 mt-1">
            {loading ? "..." : `${summary.renewalRate.toFixed(1)}%`}
          </p>
        </div>
        <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
          <p className="text-sm font-medium text-slate-500">Churn Rate</p>
          <p className="text-2xl font-bold text-slate-900 mt-1">
            {loading ? "..." : `${summary.churnRate.toFixed(1)}%`}
          </p>
        </div>
        <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
          <p className="text-sm font-medium text-slate-500">MRR</p>
          <p className="text-2xl font-bold text-slate-900 mt-1">
            {loading ? "..." : formatCurrency(summary.monthlyRevenue)}
          </p>
        </div>
      </div>
              ...
              {loading ? (
                ...
                subscriptions.map((sub) => (
                  <tr key={sub.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-sm text-slate-600">{sub.plan_name}</td>
