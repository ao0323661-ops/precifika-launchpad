...
function WebhookLogsPage() {
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'duplicated': return <Clock className="h-4 w-4 text-slate-400" />;
  ...
  const getStatusClass = (status: string) => {
    switch (status) {
      ...
      case 'duplicated': return 'bg-slate-50 text-slate-700 border-slate-100';
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50/50">
                  <th className="px-6 py-4 text-xs font-semibold uppercase tracking-wider text-slate-500">Evento</th>
                ...
                {loading ? (
                  ...
                  logs.map((log) => (
                    <React.Fragment key={log.id}>
                      <tr className="hover:bg-slate-50 transition-colors cursor-pointer" onClick={() => setExpandedId(expandedId === log.id ? null : log.id)}>
                        <td className="px-6 py-4">
                          <div className="text-sm font-medium text-slate-900">{log.event_type}</div>
                          <div className="text-[10px] font-mono text-slate-400">ID: {log.event_id || log.id.substring(0, 8)}</div>
                        </td>
                        <td className="px-6 py-4">
                          <div className={`inline-flex items-center gap-1.5 rounded-full border px-2 py-0.5 text-[11px] font-medium ${getStatusClass(log.status)}`}>
                        ...
                        <td className="px-6 py-4 text-right">
                          {expandedId === log.id ? <ChevronUp className="h-4 w-4 ml-auto text-slate-400" /> : <ChevronDown className="h-4 w-4 ml-auto text-slate-400" />}
                        </td>
                      </tr>
                      {expandedId === log.id && (
                            ...
                            <div className="space-y-4">
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div className="space-y-1">
                                  <p className="text-[10px] font-bold text-slate-400 uppercase">Payload Completo</p>
                                  <pre className="max-h-64 overflow-auto rounded-lg bg-slate-900 p-3 text-[10px] text-slate-300">
                                    {JSON.stringify(log.payload, null, 2)}
                                  </pre>
                                </div>
                                <div className="space-y-1">
                                  <p className="text-[10px] font-bold text-slate-400 uppercase">Metadados</p>
                                  <div className="rounded-lg border border-slate-200 bg-white p-3 text-xs space-y-2">
                                    <div className="flex justify-between">
                                      <span className="text-slate-500">Gateway:</span>
                                      <span className="font-medium text-slate-900">{log.gateway_provider}</span>
                                    </div>
                                    <div className="flex justify-between">
                                    ...
                                    <div className="flex justify-between">
                                      ...
                                      <span className="font-mono text-[10px] text-slate-600">{log.user_id || 'Não identificado'}</span>
                                    </div>
                                  </div>
                                </div>
                              </div>
...
// Helper to use React in the Fragment
