  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-slate-500 mt-1">Bem-vindo ao Precifika. Aqui está um resumo do seu negócio.</p>
      </div>

      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
        {[
          { label: "Receita Total", value: "R$ 12.450,00", change: "+12.5%", color: "text-emerald-600" },
          ...
          { label: "Pagamentos Pendentes", value: "R$ 1.200,00", change: "-2%", color: "text-amber-600" },
        ].map((stat) => (
          <div key={stat.label} className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm hover:shadow-md transition-shadow">
            ...
            <div className="mt-2 flex items-baseline justify-between">
              ...
              <span className={`text-xs font-bold px-2 py-0.5 rounded-full bg-slate-50 ${stat.color}`}>
                {stat.change}
              </span>
            </div>
          </div>
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-8 text-center space-y-4">
        ...
        <p className="text-slate-500 max-w-md mx-auto">
          Este é um ambiente de demonstração com dados simulados para ilustrar as funcionalidades da plataforma.
        </p>
      </div>
    </div>
  );
